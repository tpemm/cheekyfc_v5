...
def main()->int:
    p=argparse.ArgumentParser();p.add_argument('--manifest',required=True);p.add_argument('--whoscored-match-id',type=int,required=True);p.add_argument('--season',default='2627');p.add_argument('--force',action='store_true');a=p.parse_args()
    ...
    try:
        ...
        base=ROOT/f'data/raw/whoscored/{a.season}/poc';reader=sd.WhoScored(leagues='ENG-Premier League',seasons=a.season,no_cache=False,data_dir=base/'_soccerdata_native',headless=False)
        reader.read_events(match_id=a.whoscored_match_id,output_fmt=None,force_cache=True,on_error='raise')
