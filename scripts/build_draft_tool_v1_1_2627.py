...
def load_current_fantrax(path: Path) -> pd.DataFrame:
    raw = read_csv(path)
    id_col = choose_column(raw, ["api_player_id", "fantrax_player_id", "player_id"], required=True)
    name_col = choose_column(raw, ["api_player_name", "fantrax_player_name", "player_name", "name"], required=True)
    team_col = choose_column(raw, ["api_team_code", "team_code", "team", "team_short_name"], required=True)
    pos_col = choose_column(raw, ["api_position", "position", "positions"], required=True)
    ...
    adp_col = choose_column(raw, [
        "adp", "fantrax_adp", "average_draft_position", "avg_draft_position",
        "avgdraftposition", "draft_position"
    ])
    rank_col = choose_column(raw, [
        "adp_rank", "fantrax_adp_rank", "draft_rank", "consensus_rank", "rank"
    ])
    ...
    output["fantrax_adp_rank"] = pd.to_numeric(raw[rank_col], errors="coerce") if rank_col else np.nan
    output["rostered_pct"] = pd.to_numeric(raw[rostered_col], errors="coerce") if rostered_col else np.nan
...
def main() -> None:
    ...
    current = load_current_fantrax(fantrax_path)
    ...
    print("\nDraft Tool v1 data build complete.")
