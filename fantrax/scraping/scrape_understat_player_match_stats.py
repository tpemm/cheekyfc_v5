...
def read_player_match_stats_compat(us: sd.Understat, match_ids: list[int], force_cache: bool) -> pd.DataFrame:
    """
    Compatibility wrapper for soccerdata versions.

    Some versions accept:
        read_player_match_stats(match_id=..., force_cache=...)

    Other versions do NOT accept force_cache for this method.
    """
    try:
        return us.read_player_match_stats(match_id=match_ids, force_cache=force_cache)
    except TypeError as exc:
        if "force_cache" not in str(exc):
            raise
        print("NOTE: soccerdata Understat.read_player_match_stats() does not accept force_cache in this installed version.")
        print("      Retrying without force_cache...")
        return us.read_player_match_stats(match_id=match_ids)
...
def scrape_understat_epl(cfg: UnderstatScrapeConfig) -> None:
    ...
    # -------------------------
    player_ms = read_player_match_stats_compat(
        us=us,
        match_ids=new_game_ids,
