...
def acquire(rows:pd.DataFrame,*,root:Path,manifest_path:Path,timeout_seconds:float=120,retries:int=1,
    ...
    python=Path(sys.executable);all_rows=rows.to_dict('records');batch_started=time.perf_counter()
    def emit():
    ...
    for row in all_rows:
        ...
        attempts=0;timeouts=0;elapsed=0.0;last={};status='FAILED'
        for attempt in range(retries+1):
            ...
            if season=='2627':
                command=[str(python),str(root/'scripts/refresh_whoscored_live_match.py'),'--manifest',str(manifest_path),'--whoscored-match-id',str(int(row['whoscored_match_id'])),'--season',season,'--force']
            else:
            ...
            checked=validate_cache(base,row)
            if checked['cache_valid']:status='ACQUIRED';break
        ...
        message=checked.get('error_message'); message=last.get('stderr') if pd.isna(message) else message
        results.append({**common,'status':status,'worker_mode':'SESSION_BACKED' if session_backed else 'ISOLATED_PROFILE','attempt_count':attempts,'timeout_count':timeouts,'last_attempt':datetime.now(timezone.utc).isoformat(),'acquisition_seconds':round(elapsed,3),**checked,
          'old_payload_hash':old_checked.get('payload_hash'),'changed':bool(old_checked.get('payload_hash')!=checked.get('payload_hash')) if old_checked.get('cache_valid') and checked.get('cache_valid') else pd.NA,'snapshot_path':str(archive) if archive else pd.NA,'error_type':'TimeoutExpired' if status=='TIMEOUT' else checked.get('error_type'),'error_message':'worker exceeded process timeout' if status=='TIMEOUT' else message})
        emit()
