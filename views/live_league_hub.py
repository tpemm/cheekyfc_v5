...
def form_presentation(value:Any)->str:
    ...
    for token in str(value).split():
        letter="D" if token=="T" else token
        badges.append(f'<span class="hub-form hub-form-{letter.lower()}">{letter}</span>' if letter in {"W","L","D"} else escape(letter))
    return " ".join(badges)


TABLE_HELP={
    "Pts / GW":"Average Fantrax fantasy points scored per included gameweek.",
    "Ghost / GW":"Average Ghost Points from the manager's active fantasy lineup per included gameweek. Ghost Points are Fantrax points excluding points generated by Goals, Assists, and Clean Sheets.",
    "Efficiency":"Actual active-lineup Fantrax points as a percentage of the optimal eligible XI score.",
    "Lineup Changes":"Number of active-lineup changes between consecutive included gameweeks. The first gameweek has no prior lineup and is excluded.",
    "Movement":"Change in league-table position from the previous included gameweek.",
    "Form":"Most recent league matchup results, ordered from oldest to newest within the displayed form window.",
}
TABLE_WIDTHS={"Rank":"42px","Team":"200px","Movement":"82px","Record":"70px","Form":"150px",
              "Pts / GW":"90px","Ghost / GW":"96px","Efficiency":"94px","Lineup Changes":"120px"}
TABLE_CSS="""<style>
.hub-table-wrap{overflow-x:auto}
.hub-table-wrap table{border-collapse:collapse;width:auto;font-size:.9rem}
.hub-table-wrap th,.hub-table-wrap td{padding:8px 10px;border-bottom:1px solid var(--ft-border);white-space:nowrap;text-align:right}
.hub-table-wrap th:nth-child(2),.hub-table-wrap td:nth-child(2){text-align:left;white-space:normal;min-width:180px}
.hub-table-wrap th[scope=col]{font-weight:600}
.hub-form{display:inline-flex;align-items:center;justify-content:center;width:22px;height:24px;border-radius:5px;font-size:.8rem;font-weight:700;margin-right:2px}
.hub-form-w{background:#dcfce7;color:#166534;border:1px solid #86efac}
.hub-form-l{background:#fee2e2;color:#991b1b;border:1px solid #fca5a5}
.hub-form-d{background:#fef3c7;color:#92400e;border:1px solid #fcd34d}
</style>"""


def style_league_table(table:pd.DataFrame):
    styled=table.copy().style.format(escape="html",na_rep="—").map(movement_style,subset=["Movement"])
    styled=styled.format(form_presentation,subset=["Form"],na_rep="—")
    for column in ("Pts / GW","Ghost / GW","Efficiency","Lineup Changes"):
        if column not in table:continue
        styled=styled.format("{:.1f}%" if column=="Efficiency" else "{:.1f}",subset=[column],na_rep="—")
    styled=styled.format("{:.0f}",subset=["Rank"],na_rep="—")
    def header(column):
        label=escape(str(column));help_text=TABLE_HELP.get(column)
        return f'<span title="{escape(help_text,quote=True)}">{label}</span>' if help_text else label
    return styled.format_index(header,axis=1).hide(axis="index").set_table_styles([
        {"selector":f"th.col{i},td.col{i}","props":[("min-width",TABLE_WIDTHS.get(column,"90px"))]}
        for i,column in enumerate(table.columns)])


def league_table_html(table:pd.DataFrame)->str:
    # Native title tooltips and HTML badges; no JavaScript/tooltip framework.
    return TABLE_CSS+'<div class="hub-table-wrap">'+style_league_table(table).to_html()+'</div>'
...
def render(season_id:str,*,data_manager:DataManager|None=None,season_manager:SeasonManager|None=None,ui:Any=st)->None:
    ...
    section_header(ui,"League Table");table=compact_league_table(model["summary"],weeks);ui.markdown(league_table_html(table),unsafe_allow_html=True)
    ...
    ui.caption("Season records across all included completed league gameweeks; provisional results are excluded.")
